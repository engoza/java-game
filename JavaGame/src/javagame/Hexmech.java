...
//I think that this XYvertex stuff is taken care of in the int x line above. Why is it here twice?
     ...
     * ***************************************************************************
     */
    public static void fillHex(int i, int j, int n, boolean seleccionada, Graphics2D g2) {
        char c = 'o';
        int x = i * (s + t);
        int y = j * h + (i % 2) * h / 2;
    
        //g2.setColor(Hexgame.COLORES[n]);
            g2.setPaint(Hexgame.texturas[n]);
        g2.fillPolygon(hex(x, y));
        g2.setColor(Hexgame.COLOURONETXT);
        if (seleccionada) {
            g2.setColor(Hexgame.SELECTED);
        } else {
            g2.setColor(Hexgame.COLOURGRID);
        }
